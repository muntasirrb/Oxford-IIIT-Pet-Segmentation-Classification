...
HISTORY_PATH      = "./checkpoints_seg_only/history.json"
TEST_RESULTS_PATH = "./checkpoints_seg_only/test_results.json"
SAVE_DIR          = "./checkpoints_seg_only"
...
def plot_loss_curves(history, test, epochs):
    ...
    plot_single_curve(axes[1], epochs,
                      history["train_seg_loss"], history["val_seg_loss"],
                      None,
                      "Segmentation Loss", "Loss", lower_better=True)

    plot_single_curve(axes[2], epochs,
...
# ── Figure 2: Segmentation Metrics Curves ────────────────────────────────────

def plot_segmentation_metrics(history, test, epochs):
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    fig.suptitle("Training & Validation — Segmentation Metrics",
                 fontsize=14, fontweight="bold")

    plot_single_curve(axes[0], epochs,
                      history["train_pixel_acc"], history["val_pixel_acc"],
                      test.get("test_pixel_acc"),
    ...
    plt.tight_layout()
    path = os.path.join(SAVE_DIR, "plot_segmentation_metrics.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")


# ── Figure 3: Classification Metrics Curves ───────────────────────────────────
...
# ── Figure 6: Confusion Matrix (normalized only) ──────────────────────────────
...
def plot_summary(test):
    metrics = {
        "Pixel Acc" : test["test_pixel_acc"],
        "Mean IoU"  : test["test_iou"],
        "Dice"      : test["test_dice"],
        "Top-1 Acc" : test["test_top1"],
    ...
    colors = ["steelblue" if v >= 0.75 else
              "gold"      if v >= 0.50 else
              "tomato" for v in values]
    ...
    ax.set_title("Test Set — Overall Results Summary",
                 fontsize=14, fontweight="bold")
    ax.set_ylabel("Score", fontsize=11)
    ax.set_ylim(0, 1.15)
    ax.grid(True, axis="y", alpha=0.3)
    ...
    plt.tight_layout()
    path = os.path.join(SAVE_DIR, "plot_summary.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")


# ── Figure 8: Segmentation Metrics Bar Chart ──────────────────────────────────

def plot_segmentation_bar(test):
    metrics = {
        "Pixel Accuracy": test["test_pixel_acc"],
        "Mean IoU"      : test["test_iou"],
        "Dice Score"    : test["test_dice"],
    }
    ...
    ax.set_title("Segmentation Metrics — Test Set",
                 fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    path = os.path.join(SAVE_DIR, "plot_seg_bar.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")


# ── Figure 9: Classification Metrics Bar Chart ────────────────────────────────
...
# ── Figure 10: Confusion Matrix With Numbers ──────────────────────────────────
...
def plot():
    ...
    plot_segmentation_metrics (history, test, epochs)
    plot_segmentation_bar     (test)
    plot_summary              (test)
